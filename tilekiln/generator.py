...
def setup(
    config: Config, layers: Optional[set[str]], source_kwargs, storage_kwargs
...
def generate(
    ...
    with mp.Pool(
        num_processes, setup, (config, layers, source_kwargs, storage_kwargs)
    ) as pool:
...
def generate_tilelayers(
    config: Config,
    source_kwargs,
    storage_kwargs,  # type: ignore[no-untyped-def]
    tilelayers: Collection[tuple[Tile, set[str]]],
    num_processes: int,
) -> None:
    ...
    with mp.Pool(
        num_processes, setup, (config, None, source_kwargs, storage_kwargs)
    ) as pool:
