...
class Tileset:
    ...
    def from_id(cls, storage: Storage, id: str) -> Tileset:
        ...
        # Make sure we're using the correct value from the maxzoom
        # Ensure it's correctly passed to the constructor
    ...
    def prepare_storage(self) -> None:
        self.storage.create_tileset(self.id, self.layers, self.minzoom, self.maxzoom, self.tilejson)

    def update_storage_metadata(self) -> None:
        """Sets the metadata in storage"""
        self.storage.set_metadata(self.id, self.layers, self.minzoom, self.maxzoom, self.tilejson)
