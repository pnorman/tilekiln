...
class Storage:
    ...
    def create_tileset(
        self, id: str, layers: list[str], minzoom: int, maxzoom: int, tilejson: str
    ) -> None:
        with self.__pool.connection() as conn:
            with conn.cursor() as cur:
    ...
    def remove_tileset(self, id: str) -> None:
        with self.__pool.connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f'''DELETE FROM "{self.__schema}"."{METADATA_TABLE}" WHERE id = %s''',
                    (id,),
                )
                cur.execute(f'''DROP TABLE "{self.__schema}"."{id}" CASCADE''')
                cur.execute(
                    f'''DELETE FROM "{self.__schema}"."{TILE_STATS_TABLE}" WHERE id = %s''',
                    (id,),
                )
            conn.commit()
    ...
    def get_tileset(self, id: str) -> Tileset:
        ...
        with self.__pool.connection() as conn:
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                cur.execute(
                    sql.SQL(
                        """SELECT id, layers, minzoom, maxzoom, tilejson FROM {}.{}"""
                    ).format(
                        sql.Identifier(self.__schema), sql.Identifier(METADATA_TABLE)
                    )
                    + sql.SQL("""WHERE id = %s"""),
                    (id,),
                )
    ...
    def get_layer_ids(self, id: str) -> list[str]:
        ...
        with self.__pool.connection() as conn:
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                cur.execute(
                    sql.SQL("""SELECT layers FROM {}.{}""").format(
                        sql.Identifier(self.__schema), sql.Identifier(METADATA_TABLE)
                    )
                    + sql.SQL("""WHERE id = %s"""),
                    (id,),
        ...
        with self.__pool.connection() as conn:
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                ...
                if result is None:
                    ...
                    click.echo(
                        f"Failed to retrieve tilejson for id {id}, "
        ...
        with self.__pool.connection() as conn:
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                ...
                if result is None:
                    # TODO: raise exception and handle it at the calling level
                    click.echo(
                        f"Failed to retrieve minzoom for id {id}, "
                        f"does it exist in storage DB?",
                        err=True,
                    )
                    sys.exit(1)
    ...
    def get_tile(
        self, id: str, tile: Tile
    ) -> tuple[dict[str, bytes | None], datetime.datetime | None]:
        ...
        with self.__pool.connection() as conn:
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                ...
                return {
                    layer: result[f"{layer}_data"] for layer in tileset.layers
                }, result["generated"]

    def get_tile_details(
        self, id: str, tile: Tile
    ) -> dict[str, tuple[bytes, datetime.datetime] | None]:
            ...
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                ...
                for layer in tileset.layers:
                    if result is not None and result[f"{layer}_data"] is not None:
                        details[layer] = (
                            result[f"{layer}_data"],
                            result[f"{layer}_generated"],
                        )
                    else:
                        details[layer] = None
                return details
    ...
    def save_tile(
        self, id: str, tile: Tile, layers: dict[str, bytes], render_time=0
    ) -> datetime.datetime | None:
        tileset = self.get_tileset(id)
        if tile.zoom > tileset.maxzoom or tile.zoom < tileset.minzoom:
            raise tilekiln.errors.ZoomNotDefined
        if set(layers.keys()) - set(tileset.layers):
            raise tilekiln.errors.Error(
                "Rendered tile contains layers not known to storage"
            )
            ...
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                ...
                data_upsert = [
                    sql.SQL("{} = EXCLUDED.{}").format(
                        sql.Identifier(f"{layer}_data"), sql.Identifier(f"{layer}_data")
                    )
                    for layer in layers
                ]
                time_upsert = [
                    sql.SQL(
                        "{generated} = CASE WHEN store.{data} IS "
                        ...
                        generated=sql.Identifier(f"{layer}_generated"),
                ...
                q = (
                    sql.SQL("INSERT INTO {}.{} AS store\n").format(
                        sql.Identifier(self.__schema), sql.Identifier(tablename)
                    )
                    + sql.SQL("(zoom, x, y, ")
                    + _data_columns(layers)
                    + sql.SQL(", ")
                    + _generated_columns(layers)
                    + sql.SQL(")\n")
                    + sql.SQL("VALUES ({}, {}, {}, ").format(
                        sql.Literal(tile.zoom), sql.Literal(tile.x), sql.Literal(tile.y)
                    )
                    + sql.SQL(", ").join(data_idents)
                    + sql.SQL(",\n")
                    + sql.SQL(", ").join(
                        [sql.SQL("statement_timestamp()") for _ in layers]
                    )
                    + sql.SQL(")\n")
    ...
    def __setup_metadata(self, cur) -> None:
        ...
        cur.execute(
            f'''CREATE TABLE IF NOT EXISTS "{self.__schema}"."{METADATA_TABLE}" (\n'''
            """id text PRIMARY KEY,\n"""
    ...
    def __set_metadata(
        self, cur, id: str, layers: list[str], minzoom, maxzoom, tilejson
    ):
        """
    ...
    def __update_tileset_metrics(self, cur, tileset: Tileset) -> None:
        ...
        for zoom in range(minzoom, maxzoom + 1):
            ...
            length = sql.SQL("+").join(
                [
                    sql.SQL("length({})").format(sql.Identifier(f"{layer}_data"))
                    for layer in tileset.layers
                ]
            )
            ...
            query = (
                sql.SQL("INSERT INTO {}.{}\n").format(
                    sql.Identifier(self.__schema), sql.Identifier(TILE_STATS_TABLE)
                )
                ...
                + sql.SQL(
                    "array_fill(0, ARRAY[array_length(%(percentile)s, 1)]))] "
                    + "AS percentiles\n"
                ).format(length, length)
                + sql.SQL("FROM {}.{}").format(
                    sql.Identifier(self.__schema), sql.Identifier(f"{id}_z{zoom}")
                )
                + sql.SQL(
                    "ON CONFLICT (id, zoom)\n"
    ...
    def __setup_tables(
        self, cur, id: str, layers: list[str], minzoom: int, maxzoom: int
    ) -> None:
        ...
        columns = [
            sql.SQL("zoom smallint CHECK (zoom >= {} AND zoom <= {})").format(
                sql.Literal(minzoom), sql.Literal(maxzoom)
            ),
        ...
        columns += [
            sql.SQL("{} timestamptz").format(sql.Identifier(f"{layer}_generated"))
            for layer in layers
        ]
        columns += [
            sql.SQL("{} bytea").format(sql.Identifier(f"{layer}_data"))
            for layer in layers
        ]
    ...
    def __delete_tile(self, cur, id: str, tile: Tile):
        ...
        """
        cur.execute(
            f'''DELETE FROM "{self.__schema}"."{id}"
    ...
    def __delete_tilelayer(self, cur, id: str, tile: Tile, layers: set[str]):
        ...
        data = sql.SQL(", ").join(
            [
                sql.SQL("{} = NULL").format(sql.Identifier(f"{layer}_data"))
                for layer in layers
            ]
        )
        time = sql.SQL(", ").join(
            [
                sql.SQL("{} = NULL").format(sql.Identifier(f"{layer}_generated"))
                for layer in layers
            ]
        )
        cur.execute(
...
def _data_columns(layers) -> sql.Composable:
    return sql.SQL(", ").join([sql.Identifier(f"{layer}_data") for layer in layers])


def _generated_columns(layers) -> sql.Composable:
    return sql.SQL(", ").join(
        [sql.Identifier(f"{layer}_generated") for layer in layers]
    )
