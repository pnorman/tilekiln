...
class Storage:
    ...
    def get_tilesets(self) -> Iterator[Tileset]:
        ...
        with self.__pool.connection() as conn:
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                cur.execute(
                    f'''SELECT id, layers, minzoom, maxzoom, tilejson
    ...
    def get_tileset(self, id: str) -> Tileset:
        ...
        with self.__pool.connection() as conn:
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                cur.execute(
                    sql.SQL("""SELECT id, layers, minzoom, maxzoom, tilejson FROM {}.{}""").format(
    ...
    def metrics(self) -> Collection[Metric]:
        with self.__pool.connection() as conn:
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                cur.execute(
                    f'''SELECT id, zoom, num_tiles, size, percentiles
        ...
        with self.__pool.connection() as conn:
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                ...
                if result is None:
                    ...
                    click.echo(
                        f"Failed to retrieve tilejson for id {id}, does it exist in storage DB?",
                        err=True,
                    )
                    sys.exit(1)
        ...
        with self.__pool.connection() as conn:
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                ...
                if result is None:
                    # TODO: raise exception and handle it at the calling level
                    click.echo(
                        f"Failed to retrieve minzoom for id {id}, does it exist in storage DB?",
                        err=True,
                    )
                    sys.exit(1)
    ...
    def get_tile(
        ...
        with self.__pool.connection() as conn:
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                ...
                if result is None:
                    return None, None
                return {layer: result[f"{layer}_data"] for layer in tileset.layers}, result[
                    "generated"
                ]
    ...
    def save_tile(
        ...
        with self.__pool.connection() as conn:
            with conn.cursor(row_factory=psycopg.rows.dict_row) as cur:
                ...
                time_upsert = [
                    sql.SQL(
                        "{generated} = CASE WHEN store.{data} != EXCLUDED.{data} "
    ...
    def __setup_stats(self, cur) -> None:
        ...
        cur.execute(
            f"""CREATE TABLE IF NOT EXISTS "{self.__schema}"."{TILE_STATS_TABLE}" (
            id text,
            zoom smallint,
    ...
    def __update_tileset_metrics(self, cur, tileset: Tileset) -> None:
        ...
        for zoom in range(minzoom, maxzoom + 1):
            ...
            query = (
                ...
                + sql.SQL(
                    "array_fill(0, ARRAY[array_length(%(percentile)s, 1)]))] " + "AS percentiles\n"
    ...
    def __setup_tables(self, cur, id: str, layers: list[str], minzoom: int, maxzoom: int) -> None:
        ...
        columns += [
            sql.SQL("{} timestamptz DEFAULT statement_timestamp()").format(
                sql.Identifier(f"{layer}_generated")
            )
            for layer in layers
        ]
        columns += [sql.SQL("{} bytea").format(sql.Identifier(f"{layer}_data")) for layer in layers]
...
def generated_columns(layers) -> sql.Composable:
    return sql.SQL(", ").join([sql.Identifier(f"{layer}_generated") for layer in layers])
