...
@click.option(
    "--base-url",
    help="Defaults to http://127.0.0.1:8000" + " or the bind host and port",
)
...
def live(
    ...
    uvicorn.run(
        "tilekiln.server:live", host=bind_host, port=bind_port, workers=num_threads
    )
...
def static(
    ...
    uvicorn.run(
        "tilekiln.server:server", host=bind_host, port=bind_port, workers=num_threads
    )
