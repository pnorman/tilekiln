...
class TestDefinition(TestCase):
    def test_attributes(self):
        with MemoryFS() as fs:
            ...
            d = Definition(
                "bar", {"minzoom": 2, "maxzoom": 4, "file": "blank.sql.jinja2"}, fs
            )
    ...
    def test_attribute_exceptions(self):
        with MemoryFS() as fs:
            ...
            self.assertRaisesRegex(
                ...
                Definition,
                "foo",
                {"minzoom": 1, "extent": 1024, "buffer": 8, "file": "blank.sql.jinja2"},
                fs,
            )
            self.assertRaisesRegex(
                DefinitionError,
                ...
                Definition,
                "foo",
                {"maxzoom": 1, "extent": 1024, "buffer": 8, "file": "blank.sql.jinja2"},
                fs,
            )
            self.assertRaisesRegex(
                DefinitionError,
